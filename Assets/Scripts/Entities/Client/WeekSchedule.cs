...
namespace NightDriver.Client
{
    ...
    public sealed class WeekSchedule : ScriptableObject
    {
        ...
        /// <summary>
        /// 해당 일차에 손님이 한 명 이상 등록돼 있으면 그 수를, 없으면 <paramref name="fallback"/>을 반환합니다.
        /// (NightConfig.callsPerNight와 스케줄 인원을 맞출 때 사용)
        /// </summary>
        public int ResolveCallsLimitForDay(int dayIndex, int fallback)
        {
            int n = GetClientCountFor(dayIndex);
